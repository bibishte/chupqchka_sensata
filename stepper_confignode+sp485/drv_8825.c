...
{
	RealSpeed.MaxSpeed = eeprom_read_word( ( uint16_t * ) ( &max_speed ) );
	RealSpeed.MinSpeed = 5;
	RealSpeed.Acceleration = eeprom_read_word( ( uint16_t * ) ( &acceleration ) );
	StepsPerRev = 200;
	CurrentMode = eeprom_read_byte( ( uint8_t * ) ( &current_mode ) );
	CurrentDecay = eeprom_read_byte( ( uint8_t * ) ( &current_decay ) );
	if ( ( RealSpeed.MaxSpeed == 0 ) || ( RealSpeed.MaxSpeed == 0xffff ) )
	{
		RealSpeed.MaxSpeed = 30;
		Decay( AUTO_DECAY );
...
void store( void )
{
	...
	eeprom_write_word( (uint16_t *) &max_speed, RealSpeed.MaxSpeed );
	eeprom_write_word( (uint16_t *)  &acceleration, RealSpeed.Acceleration );
	//eeprom_write_word( ( uint16_t * ) ( 6 ), StepsPerRev );
	eeprom_write_byte( ( uint8_t * ) &current_mode, CurrentMode );
	eeprom_write_byte( ( uint8_t * ) &current_decay, CurrentDecay );
